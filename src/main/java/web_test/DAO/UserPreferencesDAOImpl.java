...
class UserPreferencesDAOImpl implements UserPreferencesDAO {
    ...
    public UserPreferencesDAOImpl() {
        ...
        UserPreference up3 = new UserPreference(3, "Preference 02", "PValue 02", 1, calendar.getTime(), true);
        calendar.add(Calendar.DAY_OF_YEAR, 12);
        UserPreference up3a = new UserPreference(3, "Preference 02", "PValue 02-a", 1, calendar.getTime(), true);
        calendar.add(Calendar.DAY_OF_YEAR, 11);
        UserPreference up3b = new UserPreference(3, "Preference 02", "PValue 02-b", 1, calendar.getTime(), true);
        calendar.add(Calendar.DAY_OF_YEAR, 11);
        UserPreference up3c = new UserPreference(3, "Preference 02", "PValue 02-c", 1, calendar.getTime(), true);
        calendar.add(Calendar.DAY_OF_YEAR, 1);
        UserPreference up4 = new UserPreference(4, "Preference 03", "PValue 03", 2, calendar.getTime(), true);
        calendar.add(Calendar.DAY_OF_YEAR, 9);
        UserPreference up5 = new UserPreference(5, "Preference 04", "PValue 04", 1, calendar.getTime(), false);
    ...
    public List<UserPreference> getAllUserPreferences4User(int user_id) {
        return filterUserPreferences(listOfUserPreferences, user_id, false, null);
    }
        ...
        userPre.forEach(up -> {
            if (history) {
                ...
                if (search == null) {
                    ...
                    if (userID.test(up) && active.test(up) && searchKey.test(up)) {
                        userPreFiltered.add(up);
                    }
                }
            }
